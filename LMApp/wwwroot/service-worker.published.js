...
async function onInstall(event) {
    ...
    var appSettingData = await fetch('appsettings.json')
    appSettings = await appSettingData.json();
    ...
    if (appSettings.CdnHost) {
        assetsRequests = self.assetsManifest.assets
            .filter(asset => offlineAssetsInclude.some(pattern => pattern.test(asset.url)))
            .filter(asset => !offlineAssetsExclude.some(pattern => pattern.test(asset.url)))
            .map(asset => {
                if (asset.url == "appsettings.json"
                    || asset.url == "index.html"
                    || asset.url == "favicon.ico"
                    || asset.url == "favicon.png") {
                    return new Request(asset.url, { integrity: asset.hash, cache: 'no-cache' })
                }
                return new Request(appendCdn(asset.url), { integrity: asset.hash, cache: 'no-cache' })
            });
    }
    else {
        assetsRequests = self.assetsManifest.assets
    ...
    assetsRequests.push(new Request("https://js.monitor.azure.com/scripts/b/ai.3.gbl.min.js", { cache: 'no-cache' }));
